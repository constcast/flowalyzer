...
class RRDGenerator:
	...
	def checkRRD(self, currTime):
		for i in range(0, len(self.intervals)):
			...
			if (currTime > (self.intervalStart[i] + self.intervals[i] * self.stepSize)):
				...
				self.createImage('imgs/', currTime)
	...
	def createImage(self, imgDir, endTime):
		pngImg = imgDir + '/%s-%d.png'
		for i in range(0, len(self.intervals)):
			rrdName = self.rrdDBName % (self.reportname, i)
			command = "rrdtool graph %s -imgformat PNG --end %d --start %d-%dm DEF:ds0b=%s:s:MAX LINE1:ds0b#9999FF:\"min/max\" DEF:ds0c=%s:s:MIN LINE1:ds0c#9999FF DEF:ds0a=%s:s:AVERAGE LINE2:ds0a#0000FF:\"average\"" % (pngImg % (self.reportname, self.intervals[i]), endTime, endTime, self.graphHistory[i], rrdName, rrdName, rrdName)
			os.system(command)
			print command
